...
//	inline void WriteLTTransform(const Transform &tfValue) { WriteType( tfValue ); }
...
/*
#ifdef _CAEU
		g_LTClient = *(CLTClient**)0x377C97A8;
#else
		g_LTClient = *(CLTClient**)/*0x377A0198*///0x377AF440;
//#endif

//		g_LTClient = *(CLTClient**)0x377CA7A8;
		g_CommonLT = g_LTClient->Common();
		LTRESULT nResult = g_CommonLT->CreateMessage(pMsg);
		if (nResult == LT_OK)
			m_pMsg = pMsg;
		//ASSERT(nResult == LT_OK); <- the link error help meh plz X(
	}
	inline void Term()
	{
		m_pMsg = NULL;
